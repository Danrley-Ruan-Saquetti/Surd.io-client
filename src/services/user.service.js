...
export default function UserService() {
    ...
    // User
    const getUsers = ({ res = () => {}, isLobby = false }) => {
        socket.emit(`users${isLobby ? "/connected" : ""}`, authService.getToken())

        socket.on(`users${isLobby ? "/connected" : ""}/res`, (data) => {
            res(data)
            socket.off(`users${isLobby ? "/connected" : ""}/res`)
        })
    }
